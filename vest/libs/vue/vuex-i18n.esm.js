...
let i18nVuexModule = {
  ...
  mutations: {
    ...
    ADD_LOCALE: function ADD_LOCALE(state, payload) {
      ...
      let translations = flattenTranslations(payload.translations);

      if (state.translations.hasOwnProperty(payload.locale)) {
        // get the existing translations
        let existingTranslations = state.translations[payload.locale]; // merge the translations
    ...
    REPLACE_LOCALE: function REPLACE_LOCALE(state, payload) {
      // reduce the given translations to a single-depth tree
      let translations = flattenTranslations(payload.translations); // replace the translations entirely
    ...
    REMOVE_LOCALE: function REMOVE_LOCALE(state, payload) {
      ...
      if (state.translations.hasOwnProperty(payload.locale)) {
        ...
        let translationCopy = Object.assign({}, state.translations); // remove the given locale
...
let flattenTranslations = function flattenTranslations(translations) {
  let toReturn = {};
  ...
  for (let i in translations) {
    ...
    let objType = _typeof(translations[i]); // allow unflattened array of strings
    ...
    if (isArray(translations[i])) {
      ...
      for (let index = 0; index < count; index++) {
        let itemType = _typeof(translations[i][index]);
...
let VuexI18n = {}; // internationalization plugin for vue js using vuex

VuexI18n.install = function install(Vue, store, config) {
  ...
  let translateFilterName = config.translateFilterName;
  let translateInFilterName = config.translateInFilterName; // initialize the onTranslationNotFound function and make sure it is actually
  // a function
  ...
  let render = renderFn(identifiers, config.warnings); // get localized string from store. note that we pass the arguments passed
  ...
  let translateInLanguage = function translateInLanguage(locale) {
    ...
    let args = arguments; // initialize options

    let key = '';
    let defaultValue = '';
    let options = {};
    let pluralization = null;
    let count = args.length; // check if a default value was specified and fill options accordingly
    ...
    let translations = store.state[moduleName].translations; // get the last resort fallback from the store

    let fallback = store.state[moduleName].fallback; // split locale by - to support partial fallback for regional locales
    // like de-CH, en-UK

    let localeRegional = locale.split('-'); // flag for translation to exist or not

    let translationExists = true; // check if the language exists in the store. return the key if not
    ...
    let asyncTranslation = onTranslationNotFound(locale, key, defaultValue); // resolve async translations by updating the store
  ...
  let translateInLanguageFilter = function translateInLanguageFilter(key, locale) {
  ...
  let checkKeyExists = function checkKeyExists(key) {
    let scope = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'fallback';
    // get the current language from the store
    let locale = store.state[moduleName].locale;
    let fallback = store.state[moduleName].fallback;
    let translations = store.state[moduleName].translations; // check the current translation
...
let renderFn = function renderFn(identifiers) {
  let warnings = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  ...
  let matcher = new RegExp('' + identifiers[0] + '{1}(\\w{1}|\\w.+?)' + identifiers[1] + '{1}', 'g'); // define the replacement function
  ...
  let render = function render(locale, translation) {
    let replacements = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    let pluralization = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : null;

    // get the type of the property
    let objType = _typeof(translation);

    let pluralizationType = _typeof(pluralization);

    let resolvePlaceholders = function resolvePlaceholders() {
    ...
    let resolvedTranslation = resolvePlaceholders(); // initialize pluralizations

    let pluralizations = null; // if translations are already an array and have more than one entry,
    ...
    let index = plurals.getTranslationIndex(locale, pluralization); // check if the specified index is present in the pluralization
